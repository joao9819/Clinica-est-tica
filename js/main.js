(function () {
  ...
  function initSplash() {
    window.setTimeout(hideSplash, 4700);
  ...
  function updateServices() {
    (data.services || []).forEach(function (service) {
      ...
      if (cta) {
        ...
        var msg = (messages.servicePrefix || "Olá! Gostaria de agendar uma avaliação de") + " " + label + ".";
  ...
  function mountReviews() {
    ...
    (data.reviews || []).slice(0, 4).forEach(function (review) {
      ...
      card.innerHTML =
        ...
        "</footer>" +
        (review.placeholder ? '<span class="placeholder-label">Substituir por avaliação real</span>' : "");
  ...
  function mountGallery() {
    qsa("[data-gallery]").forEach(function (target) {
      ...
      if (!items.length) {
        ...
        empty.textContent =
          "Nenhuma imagem autorizada configurada ainda. Adicione fotos reais no manifest e marque consentimento: true.";
